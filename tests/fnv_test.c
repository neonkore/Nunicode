...
void test_fnv_lookup() {
	...
	hash = fnv_hash(FNV, sizeof(FNV) / sizeof(*FNV), 0x0061);
	assert(fnv_lookup(VALUES, 0x0061, hash) != 0);

	hash = fnv_hash(FNV, sizeof(FNV) / sizeof(*FNV), 0x0062);
	assert(fnv_lookup(VALUES, 0x0062, hash) != 0);

	hash = fnv_hash(FNV, sizeof(FNV) / sizeof(*FNV), 0x0063);
	assert(fnv_lookup(VALUES, 0x0063, hash) != 0);

	hash = fnv_hash(FNV, sizeof(FNV) / sizeof(*FNV), 0x0064);
	assert(fnv_lookup(VALUES, 0x0064, hash) != 0);

	hash = fnv_hash(FNV, sizeof(FNV) / sizeof(*FNV), 0x0065);
